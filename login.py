from constants import db_login_credentials_constants as DBKEY
from constants import db_constants as DB_CONS
from util import validator as check
from tkcalendar import Calendar
from tkextrafont import Font
from util import crytography
...
class login_GUI(ctk.CTk):
    def __init__(self):
        ...
        def signin():
            ...
            password = crytography.hash_salt(self.password_entry.get(), salt, len(self.email_entry.get()))
            print(password)
            ...
            try:
                db_con = mariadb.connect(user="root", password="test123", host="127.0.0.1", port=3306, database= DB_CONS.DB)
                db_cur = db_con.cursor()
                db_cur.execute(f'SELECT COUNT(*) AS len FROM {DBKEY.TABLE} WHERE {DBKEY.USERNAME} = ? AND {DBKEY.PASSWORD} = ?', (self.email_entry.get(), password))
        ...
        self.create_account_btn.pack(side="bottom",pady=(5,40))
        ...
        self.login_btn.pack(side="bottom",pady=(20,5))
    ...
    def create_event(self):
        ...
        def check_entry():
            ...
            elif (not check.name_format(self.fname_entry.get()) or not check.name_format(self.lname_entry.get())):
            ...
            elif not check.email_format(self.emailCreate_entry.get()):
            ...
            try:
                db_con = mariadb.connect(user="root", password="test123", host="127.0.0.1", port=3306, database= DB_CONS.DB)
                db_cur = db_con.cursor()
                db_cur.execute(f'Select * from {DBKEY.TABLE} where {DBKEY.USERNAME} = ?', (self.email_entry,))
                if(db_cur.fetchall()):
                    self.error_msg.configure(text= 'Username already exist')
                    return
                salt = base64.urlsafe_b64encode(uuid.uuid4().bytes)
                password = crytography.hash_salt(self.passCreate_entry.get(), salt, len(self.emailCreate_entry.get()))
                db_cur.execute(f'Insert into login_credentials values (?, ?, ?)', (self.emailCreate_entry.get(), password, salt))
                db_con.commit()
            except mariadb.Error:
        ...
        self.error_msg = ctk.CTkLabel(self.create_frame, text="", font=("Roboto", 18), text_color="red")
        self.error_msg.place(relx=0.5, rely=0.85, anchor="c")
        self.error_msg.place_forget()

        self.create_acc.protocol("WM_DELETE_WINDOW", close_create)
    ...
    def show_cal(self):
        ...
        self.cal = Calendar(self.cal_frame, font="Roboto 16", year=2000, month=1, day=1, showweeknumbers=False,
        ...
        self.cal_frame.protocol("WM_DELETE_WINDOW", get_date)
